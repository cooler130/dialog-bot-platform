...
import com.cooler.ai.dm.util.StringUtil;
...
public class PolicyProcessServiceImpl implements PolicyProcessService {
    ...
    public List<PolicyAction> queryPolicy(DialogState dialogState) {
        ...
        if (toState.equals(Constant.GLOBAL_ERROR)) {
            ...
            Policy defaultPolicy = policyService.selectDefaultPolicy(domainName, taskName, version);
            PolicyAction defaultAction = policyActionService.getDefaultPolicyAction(domainName, taskName, version);
            dialogState.addToParamValueMap(PC.POLICY_ID, defaultPolicy.getId() + "", Constant.PLATFORM_PARAM);
        ...
        List<Policy> policies = policyService.selectByFromToState(domainName, taskName, targetFromState, transformIntentName, toState, version);     //先匹配转移意图，此转移意图至少是原始意图。
        Policy targetPolicy = selectPolicies(policies, checkedConditionsJS);

        if (targetPolicy == null && isIntentTransformed) {
            policies = policyService.selectByFromToState(domainName, taskName, targetFromState, intentName, toState, version);   //如果发生了意图转义，则原始意图和转义意图一定不一样
        ...
        if (targetPolicy != null) {
            ...
            targetPolicy = policyService.selectDefaultPolicy(domainName, taskName, version);                                                                      //如果policies没东西，则兜底策略加进去，作为保底
        }
    ...
    public DMResponse runActions(DialogState dialogState, List<PolicyAction> actions) {
        Map[] fiveMaps = {
                ...
                dialogState.getFromModelStateMap(Constant.BIZ_PARAM_MAP, Map.class),     //前4个Map作为全局变量池
                new HashMap<String, String>()                                            //第5个Map作为局部变量池
        };
        ...
        for (PolicyAction action : actions) {
            ...
            logger.info("Action Running   : P:{} -> A: ( {}/{} ) [ actionId: {}, actionName: {}, actionType: {} ] ", policyId, ++ i, actionSize, actionId, actionName, actionType);
