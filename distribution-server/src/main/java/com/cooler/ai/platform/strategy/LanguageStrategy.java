...
import com.cooler.ai.platform.model.DomainIndicator;
                            ...
                            if(historyDialogStates != null && historyDialogStates.size() > 0){
                                for (DialogState historyDialogState : historyDialogStates) {
                                    ...
                                    if(nluDomainNames.contains(nluDomainNameTmp)){
                                        NLUIntent nluIntent = nluDomainNameIntentMap.get(nluDomainNameTmp);
                                        String[] params = getDomainIntentTaskByIntentId(nluIntent.getIntentId(), intentService, Arrays.asList(historyDialogState));
                                        domainName = params[0];
                                        intentName = params[1];
                                        taskName = params[2];
                                        isFixed = true;
